...
export function PublicHeader() {
  ...
  return <header className="border-b bg-background/95"><div className="mx-auto flex min-h-18 max-w-[1440px] items-center justify-between gap-3 px-5 py-3 lg:px-10"><Brand /><nav className="hidden items-center gap-7 text-sm font-semibold md:flex" aria-label="Primary navigation">{user?.role === "resident" && <><Link href="/resident#report">{t("report")}</Link><Link href="/resident#assistant">AI assistant</Link><Link href="/lite">{t("lite")}</Link></>}</nav><div className="flex items-center gap-2"><LanguagePicker />{user ? <><Button asChild size="sm" variant="outline"><Link href={home}>{t("myComplaints")}</Link></Button><Button size="icon" variant="ghost" aria-label={t("signOut")} onClick={logout}><LogOut /></Button></> : <Button asChild size="sm"><Link href="/login">{t("signIn")}</Link></Button>}</div></div></header>;
}
