...
export function Brand({ compact = false }: { compact?: boolean }) {
  return (
    ...
    >
      ...
      <span>
        <span className="block text-lg font-extrabold tracking-[-.04em]">
          Nivaran
        </span>
        {!compact && (
    ...
    <div className="flex border" role="group" aria-label="Interface language">
      {options.map(({ code, label }) => (
        <button
          ...
          className={`px-2 py-1.5 text-[11px] font-bold ${locale === code ? "bg-ink text-white" : "bg-background text-muted-foreground hover:text-foreground"}`}
        >
          {label}
        </button>
      ))}
...
export function PublicHeader() {
  ...
  return (
    <header className="border-b bg-background/95">
      <div className="mx-auto flex min-h-18 max-w-[1440px] items-center justify-between gap-3 px-5 py-3 lg:px-10">
        <Brand />
        <nav
...
export function TrustStrip() {
  ...
  return (
    <div className="border-y bg-secondary/45">
      <div className="mx-auto flex max-w-[1440px] flex-wrap items-center justify-between gap-3 px-5 py-3 text-xs text-muted-foreground lg:px-10">
        ...
        <span>
          {locale === "en"
            ? "No report is silently rejected by AI"
            : locale === "hi"
              ? "AI किसी शिकायत को चुपचाप अस्वीकार नहीं करता"
              : "AI कोणतीही तक्रार गुपचूप नाकारत नाही"}
        </span>
        <span>
          {locale === "en"
            ? "Reporter identity is access-controlled"
            : locale === "hi"
              ? "शिकायतकर्ता की पहचान सुरक्षित है"
              : "तक्रारदाराची ओळख प्रवेश-नियंत्रित आहे"}
        </span>
      </div>
    </div>
  );
