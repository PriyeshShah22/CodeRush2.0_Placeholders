...
import { CheckCircle2, FlaskConical, Loader2, MapPin, Paperclip, Route } from "lucide-react";
...
type TaskNotification = { complaint_id?: string; kind: string; message: string; read: boolean };
export function TaskWorkspace({ id }: { id: string }) {
  ...
  const [adminRequests, setAdminRequests] = useState<TaskNotification[]>([]);
  const [localRecoveryTargets] = useState<Record<string, string>>(() => {
    if (typeof window === "undefined") return {};
    try { return JSON.parse(localStorage.getItem("nivaran-recovery-targets") || "{}") as Record<string, string>; } catch { return {}; }
  });
  ...
  useEffect(() => {
    ...
    api<{ data: TaskNotification[] }>("/notifications")
      .then((response) => setAdminRequests(response.data.filter((item) => item.kind === "admin_update_requested")))
  ...
  async function simulateBreach() {
    if (!detail) return;
    try {
      await api(`/complaints/${detail.complaint.id}/sla/simulate`, { method: "POST", body: JSON.stringify({ stage: "department" }) });
      setDetail((current) => current ? { ...current, complaint: { ...current.complaint, status: "escalated" }, sla: current.sla ? { ...current.sla, department_breached_at: new Date().toISOString() } : current.sla } : current);
      toast.success("Department SLA breached and escalated to Admin");
    } catch (caught) {
  ...
  const matchingRequests = adminRequests.filter((item) => item.complaint_id === complaint.id);
  const hasRecoveryTarget = adminRequests.some((item) => item.complaint_id === complaint.id && item.kind === "admin_recovery_target") || Boolean(localRecoveryTargets[complaint.id]);
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <div className="mt-7 grid gap-5 lg:grid-cols-[1fr_.72fr]">
        ...
        <aside className="border bg-card p-6">
          ...
          {matchingRequests.length ? <div className="mt-5 border border-amber-200 bg-amber-50 p-3"><p className="text-xs font-bold uppercase tracking-wide text-amber-900">Admin request</p>{matchingRequests.map((item, index) => <p key={index} className="mt-1 text-xs leading-5 text-amber-900">{item.message}</p>)}</div> : null}
          <div className="mt-5 border-t pt-4">
            <p className="text-xs font-bold uppercase tracking-wide text-muted-foreground">SLA demo</p>
            <p className="mt-1 text-xs text-muted-foreground">Breach this department window to notify Admin.</p>
            <Button variant="ghost" size="sm" className="mt-2 px-0 text-amber-800 hover:text-amber-900" disabled={Boolean(sla?.department_breached_at)} onClick={() => void simulateBreach()}><FlaskConical />{sla?.department_breached_at ? "Escalated to Admin" : "Simulate breach"}</Button>
          </div>
        </aside>
      </div>
    </div>
