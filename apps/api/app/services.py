    ...
    for sla in db.scalars(select(SLARecord)).all():
        ...
        if remaining<=0 and not already_breached:
            if stage=="review": sla.review_breached_at=now
                ...
                sla.department_breached_at=now
            ...
            if c.priority!=Priority.critical:
                c.priority=priority_order[min(priority_order.index(c.priority)+1,len(priority_order)-1)]
            ...
            for admin in admins: db.add(Notification(user_id=admin.id,complaint_id=c.id,kind=f"{stage}_sla_escalation",message=message,locale="en"))
            if stage=="review":
                reviewers=db.scalars(select(User).where(User.role==Role.reviewer)).all()
                for reviewer in reviewers: db.add(Notification(user_id=reviewer.id,complaint_id=c.id,kind="review_sla_breach",message=f"Human review SLA breached for {c.reference_number}. Priority was raised to {c.priority.value}.",locale="en"))
            ...
            if identity and identity.user_id:
                db.add(Notification(user_id=identity.user_id,complaint_id=c.id,kind="complaint_delayed",message=f"Complaint no. {c.reference_number} is delayed. We have alerted the responsible team and will keep you updated.",locale=c.language if c.language in ("en","hi","mr") else "en"))
            if stage=="department":
                department_users=db.scalars(select(User).where(User.department_id==assignment.department_id)).all() if assignment else []
                for department_user in department_users: db.add(Notification(user_id=department_user.id,complaint_id=c.id,kind="department_sla_breach",message=f"Your department SLA breached for {c.reference_number}. Priority was raised to {c.priority.value}; Admin has been alerted.",locale="en"))
    db.commit()
