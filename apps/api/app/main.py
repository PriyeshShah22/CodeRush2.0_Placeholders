...
def nearby_issues(request: Request,latitude: float,longitude: float,radius: int=2000,user: User=Depends(require_roles(Role.resident)),db: Session=Depends(get_db)):
    ...
    for cluster in clusters:
        ...
        rows.append({"id":cluster.id,"title":cluster.title,"category":cluster.category,"status":complaint.status.value,"location_text":complaint.location_text,"distance_metres":round(distance),"independent_reports":cluster.duplicate_count,"affected_residents":cluster.duplicate_count+confirmations,"community_confirmations":confirmations,"already_affected":cluster.id in supported,"updated_at":complaint.updated_at})
...
def confirm_affected(request: Request,incident_id: str,body: IncidentSupportRequest,user: User=Depends(require_roles(Role.resident)),db: Session=Depends(get_db)):
    ...
    if distance is None or distance>allowed_radius: raise HTTPException(403,f"You must be within {allowed_radius} metres of this issue")
    ...
    confirmations=incident_support_count(db,incident.id); affected=incident.duplicate_count+confirmations
...
def complaint_detail(complaint_id: str,user: User=Depends(current_user),db: Session=Depends(get_db)):
    ...
    incident=None
    if incident_row:
        cluster=incident_row[1]; confirmations=incident_support_count(db,cluster.id); affected=cluster.duplicate_count+confirmations
